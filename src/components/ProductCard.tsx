...
export const ProductCard = ({ product }: ProductCardProps) => {
  return (
    <Link to={`/products/${product.slug}`} className="group block relative h-full cursor-pointer" data-cursor="discover">
        ...
        <div className="relative aspect-[4/5] overflow-hidden rounded-t-[2rem]">
          {product.images[0] ? (
            <img 
              ...
              alt={product.name}
              className="h-full w-full object-cover transition-transform duration-1000 ease-custom group-hover:scale-[1.05]"
            />
          ) : (
        ...
        <div className="p-8 flex flex-col flex-1 justify-between bg-gradient-to-b from-transparent to-background/5">
          <div>
            ...
            {product.shortDescription && (
              <SafeHtmlRenderer html={product.shortDescription} className="line-clamp-2 mt-3 text-lg leading-relaxed text-muted-foreground font-light [&_p]:m-0 [&_p]:inline" />
            )}
          </div>
        </div>
      </div>
